...
const BatchLogsList = ({ logs, onRefresh }: BatchLogsListProps) => {
  ...
  const renderLogDetails = (log: OilProcessingBatchLog) => {
    ...
    if (!details) return <div className="text-gray-500">Loading details...</div>;
    ...
    switch (log.action_type) {
      ...
      case 'transportation_plant_to_home':
        return (
          <div className="space-y-1 text-sm">
            {details.driver_name && (
              <div>Driver: {details.driver_name}</div>
            )}
            {details.price && (
      ...
      case 'processing':
        return (
          <div className="space-y-1 text-sm">
            {details.plant_name && (
              <div>Plant: {details.plant_name}</div>
            )}
            {details.weight_kgs && (
              <div>Weight: {details.weight_kgs} kg</div>
            )}
            {details.yield_liters && (
              <div>Yield: {details.yield_liters} L</div>
            )}
            {details.percentage && (
              <div>Percentage: {details.percentage}%</div>
            )}
            {details.price && (
              <div>Price: ${details.price}</div>
            )}
  ...
  const getActionIcon = (actionType: string) => {
    switch (actionType) {
      case 'transportation_grove_to_plant': return '🚚';
      case 'processing': return '⚙️';
      case 'filtering': return '🔍';
      case 'transportation_plant_to_home': return '🏠';
      case 'bottling': return '🍾';
      default: return '📝';
    }
  };
  ...
  const getActionLabel = (actionType: string) => {
    switch (actionType) {
      case 'transportation_grove_to_plant': return 'Transportation (Grove → Plant)';
      case 'processing': return 'Processing';
      case 'filtering': return 'Filtering';
      case 'transportation_plant_to_home': return 'Transportation (Plant → Home)';
  ...
  if (logs.length === 0) {
    return (
      <div className="card">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">Batch Activity Logs</h2>
        <div className="text-center py-8 text-gray-500">
          No logs recorded yet. Add your first log entry to track the oil processing progress.
        </div>
      </div>
    );
  }
